...
# fetch out the second highest marks of the students
...
select * from ineuron_students order by students_marks desc limit 2,1;
...
select student_id, student_batch, students_marks, row_number() over(order by students_marks desc) as 'ROW_NUM' from ineuron_students;

#Partition by
select student_id, student_batch, students_marks, 
row_number() over(partition by student_batch order by students_marks desc) as 'ROW_NUM' 
from ineuron_students;

#SUB QUERY
select * from (select student_id, student_batch, students_marks, 
row_number() over(partition by student_batch order by students_marks desc) as 'ROW_NUM' 
...
select * from
(select student_id, student_batch, students_marks, 
dense_rank() over(order by students_marks desc) as x
from ineuron_students) as test where x=1;